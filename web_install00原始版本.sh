# date 2017.1.21
# My 1319996958@qq.com & http://blog.amoli.cc:81
...
check_port() {
		if [ "$1" -gt "0" ]&&[ "$1" -lt "65535" ]; then
			if netstat -tlpn | grep "\b$1\b" 1>/dev/null; then
				echo ""${1}"端口被占用, 换个端口试试？"
				exit
			fi
		else
...
if [ ! -d "$WEB_PATH" ]; then
	echo "网站路径不存在 $WEB_PATH"
	exit
fi

check_port $WEB_ROOT_PORT
[ $OWNCLONUD == 1 ]&& check_port $OWNCLONUD_PORT
[ $KODEXPLORER == 1 ]&& check_port $OWNCLONUD_PORT
[ $PHPMYADMIN == 1 ]&& check_port $PHPMYADMIN_PORT
...
if [ ! -f "/tmp/weblog/web_install.log" ]; then
	...
	echo "完成" > /tmp/weblog/web_install.log
fi
...
if [ "$KODEXPLORER" == "1" ]; then
	rm $WEB_PATH/www/kodexplorer.tar.bz2
...
if [ "$OWNCLONUD" == "1" ]; then
	rm $WEB_PATH/www/owncloud.tar.bz2
...
if [ "$PHPMYADMIN" == "1" ]; then
	rm $WEB_PATH/www/phpmyadmin.tar.bz2
...
if [ "$ZBLOG" == "1" ]; then
	...
	rm $WEB_PATH/www/zblog.tar.bz2
fi

rm  $WEB_PATH/www/$PHP_VER.tar.bz2
...
# 重启 nginx php mysql
/usr/bin/www restart 2>/dev/null
